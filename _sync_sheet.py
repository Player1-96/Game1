...
def tab_fabao():
    ...
    rows.append(["说明", "★「玄元镜」是唯一一件在三个流派下用途完全不同的法宝：飞剑 / 巨剑流下是"
                 ...
                 "而是掉头打回去（伤害 = 玩家伤害 ×(0.8+0.5×镜阶)，自寻最近的妖物，"
                 "二重起可多穿透 1~2 个）。它的进阶效果也随流派变（见 upByStyle）"])
...
def tab_ult():
    ...
    rows.append(["说明", "剑影三叠：按住空格蓄势（24 帧 ≈ 0.4 秒蓄满），松手朝指针突进斩击，"
                        ...
                        "（冷却清零），二段伤害 ×1.2；二段命中后再接三段，落脚化作五连斩且必定暴击。"
    ...
    rows.append(["说明", "舞剑流的风险：蓄势期间被打断 → 剑势溃散、技能立刻进冷却；"
...
def tab_ultpath():
    ...
    rows.append(["说明", "舞剑流的 6 条路线：剑锋凌厉（伤害 +20/40/60%）、剑势绵长（突进距离 +17/33/50%，"
                        ...
                        "剑罡护体（突进后无敌 30/60/90 帧）、剑意不绝（每击杀返还 2/4/5 秒冷却 —— "
...
def tab_player():
    ...
    desc = {
        ...
        "reflect": "照影：被斩中的术法掉头打回去（仅舞剑流，来自「玄元镜」）",
        "fly": "免疫地面秽气"
    }
    for k, v in st.items():
    ...
    rows.append(["说明", "舞剑流专属「剑影三叠」：按住空格蓄势 → 松手朝指针突进（全程无敌）→ "
                        "命中后 1.4 秒内可接二段（冷却清零、伤害 ×1.2）→ 再命中后接三段（落脚五连斩、必定暴击）；"
    ...
    if uwj:
        ...
        rows.append(["说明", "舞剑流风险：蓄势被打断 → 剑势溃散、立刻进冷却；命中后未在窗口内接招 → "
                            "连招归零、冷却回满。只有低于 %d 帧（≈ %.2f 秒）的点按算误触，"
                            "收势、不位移、也不收冷却"
...
def tab_log():
    ...
    rows = [head,
            ["2026-09-15", "舞剑流「剑意不绝」改为击杀返还冷却（2/4/5 秒）+ 头目一阶段 45 秒软时限",
             "一、把「剑意不绝」从「释放时固定 −2/4/6 秒」改成击杀返还：每击杀一只妖物把专属冷却往回退 "
             "2 / 4 / 5 秒（一级 2 秒、二级 4 秒、三级 5 秒）。这是全表唯一一条「条件触发」的 CDR —— "
             ...
             "（2/4/5 秒）并给该行标注「击杀返还型」；「BOSS」页新增一阶段软时限说明；"
