...
def tab_player():
    ...
    if uwj:
        ...
        rows.append(["说明", "舞剑流风险：起手蓄势被打断 → 剑势溃散、立刻进冷却；命中后未在窗口内接招 → "
                            "连招归零、冷却回满。只有低于 %d 帧（≈ %.2f 秒）的点按算误触，"
                            "收势、不位移、也不收冷却。而「已经连上的招」不会被摸断 —— "
                            "一段命中后的接招蓄势自带无敌，三段突进与落脚的五连斩全程无敌、"
                            "收招再留 %d 帧余韵，保证五连斩一定砍得出来"
...
def tab_log():
    ...
    rows = [head,
            ["2026-09-16", "接招期的蓄势不再被打断（保证三段五连斩砍得出来）+ 五连斩收招余韵",
             ...
             "现在接招期（连段窗口内，wjStage > 0）的蓄势自带无敌，不会再被摸断；"
             "起手那一记仍保留赌注（wjStage = 0 时不给无敌），不然「敢不敢贴上去」的性格就没了。"
             "另给五连斩收招加了 %d 帧余韵无敌：五刀砍完时人正在怪堆正中，"
