...
(async () => {
    ...
    function smartMove(G, aimX, aimY, navigating, wantDist) {
      ...
      for (let di = 0; di < 8; di++) {
        ...
        for (const e of G.enemies) {
          ...
          if (d < safe) risk += (safe - d) * (mel ? 0.10 : 0.55);
        ...
        // 舞剑流战斗中：目标(贴进去/撤出来)必须有实际影响力，
        // 原来战斗中权重只有 0.005，走位完全被威胁场支配 —— 想贴也贴不进去。
        const mw = (G.style === 'wujian' && !navigating);
        const score = -risk * (mw ? 0.45 : 1) - da * (navigating ? 0.30 : (mw ? 0.11 : 0.005)) + jitter;
        ...
        if (enemies.length > 0) {
          ...
          if (p.ult && S.style === 'wujian') {
            ...
            if (p.wjCharging) {
              ...
              if (S.wjHold >= 22 || bd < 70) { try { G.ultUp(); } catch (e) { } }
            } else {
              S.wjHold = 0;
              if (p.ultCd <= 0 && bd > 90) { try { G.useUlt(); } catch (e) { } }
            }
          ...
          if (S.style === 'wujian') {
            // 近战节奏：挥砍就绪 → 贴进剑锋内(34px)；后摇期间 → 撤出接触范围
            const ready = p.shootCd <= 5;
            const aw = Math.atan2(p.y - t.y, p.x - t.x);
            if (ready && bd > 26) { tx = t.x; ty = t.y; }
            else if (!ready && bd < 52) { tx = p.x + Math.cos(aw) * 72; ty = p.y + Math.sin(aw) * 72; }
            else if (bd > 46) { tx = t.x; ty = t.y; }
            else { tx = p.x + Math.cos(aw + 1.6) * 40; ty = p.y + Math.sin(aw + 1.6) * 40; }
  ...
  fs.writeFileSync(path.join(__dirname, '..', 'data', `_bot_smart_result_${ARM}.json`), JSON.stringify({ arm: ARM, runs, errs }, null, 2));
