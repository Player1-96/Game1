...
const SHOTDIR = path.join(__dirname, '..', 'data', 'shots_smart');
...
(async () => {
  ...
  await page.evaluate(() => {
      ...
      for (let di = 0; di < 8; di++) {
        ...
        for (const b of G.bullets) {
          ...
          for (let t = 0; t <= H; t += 3) {
            ...
            if (d < 17) risk += (17 - d) * (G.style === 'wujian' ? 0.22 : 0.6);
          }
        ...
        for (const e of G.enemies) {
          ...
          // 舞剑流的剑锋够到 53px，接触圈只有 13~15px —— 没有任何理由主动钻进去。
          // 旧值给 4px 余量 + 0.10 权重，语义是「近战撞上也不怕」，是它第 1 层就躺的主因。
          const safe = (e.r || 8) + (mel ? 10 : 18);
          if (d < safe) risk += (safe - d) * (mel ? 0.42 : 0.55);
        ...
        if (ne) {
          ...
          const mel2 = (G.style === 'wujian');
          const floor = (ne.r || 8) + (mel2 ? 24 : 30);
          if (dEn < floor) risk += (floor - dEn) * 0.16;
          else if (dEn < WANT) risk += (WANT - dEn) * 0.010;
          else risk += Math.min(mel2 ? 6.0 : 1.2, (dEn - WANT) * (mel2 ? 0.05 : 0.006));
        }
        // 地面危险区
    ...
    window.__step = function (n) {
      ...
      for (let f = 0; f < n; f++) {
        ...
        if (enemies.length > 0) {
          ...
          S.mDist = (S.mDist || 0) + bd; S.mF = (S.mF || 0) + 1;
          ...
          if (p.ult && S.style === 'wujian') {
            /* 舞剑流的主输出是三段连招（一段 2.0× → 二段 ×1.2 → 三段五连斩必暴击），
               不是单发突进。旧版要求「距怪 >90px 才起手」，可命中之后怪就在身边，
               于是二段三段永远接不上 —— 每次都是「一段打完 → 窗口到期 →
               连招归零 + 冷却回满」，等于把这流派的输出砍掉 2/3。 */
            if (p.wjCharging) {
              S.wjHold = (S.wjHold || 0) + 1;
              // 蓄满即放；已进目标接触圈就提前放，免得白吃撞
              if (S.wjHold >= 22 || (S.wjHold >= 8 && bd < (t.r || 8) + 46)) { try { G.ultUp(); } catch (e) { } }
            } else {
              S.wjHold = 0;
              const canChain = p.wjChainT > 0;                       // 连段窗口内：无条件续招
              const wantOpen = p.ultCd <= 0 && bd > 58 && bd < 190;  // 起手：够得着且有冲刺距离
              if (canChain || wantOpen) { try { G.useUlt(); } catch (e) { } }
            }
          ...
          if (S.style === 'wujian') {
            /* 站位是这套流派的第一性问题：剑锋 46px + 妖物体型 ≈ 53px 够得着，
               接触圈只有 13~15px。所以正解是站在「够得着、撞不到」的 30~46px，
               而不是像旧版那样一路贴到 26px —— 那是把长兵器当匕首用。 */
            const ready = p.shootCd <= 5;
            const aw = Math.atan2(p.y - t.y, p.x - t.x);   // 「远离目标」的方向
            if (p.hp <= 2 && p.ultCd > 0) { tx = p.x + Math.cos(aw) * 96; ty = p.y + Math.sin(aw) * 96; }
            else if (bd > 46) { tx = t.x; ty = t.y; }                                              // 够不着 → 进
            else if (bd < 30) { tx = p.x + Math.cos(aw) * 64; ty = p.y + Math.sin(aw) * 64; }      // 太近 → 退
            else if (!ready) { tx = p.x + Math.cos(aw + 1.6) * 32; ty = p.y + Math.sin(aw + 1.6) * 32; }  // 后摇 → 侧移
            else { tx = p.x; ty = p.y; }                                                           // 就绪且站位好 → 原地砍
            tx = Math.max(56, Math.min(424, tx)); ty = Math.max(56, Math.min(232, ty));
          } else { tx = p.x; ty = p.y; }
          // 新版：灵力全靠地上捡 —— 战斗中也要去捡灵力珠，否则永远回不上蓝
          {
            ...
            if (orb && p.mp < p.maxMP * 0.7 && od < 190 && !(S.style === 'wujian' && bd < 60)) { tx = orb.x; ty = orb.y; }
          }   // 战斗时纯靠风险最小化走位
        } else {
        ...
        const mv = smartMove(G, aimX, aimY, navigating && G.enemies.length === 0, S.chargeIn ? 24 : (S.style === "jujian" ? 132 : (S.style === "wujian" ? 44 : 88)));
        const i2 = G.input;
    ...
    window.__report = function () {
      ...
      return {
        ...
        starve: S.starve || 0, tiers: window.__tiers || [0,0,0], avgDist: (S.mF ? +(S.mDist / S.mF).toFixed(1) : 0),
  ...
  fs.writeFileSync(path.join(__dirname, '..', 'data', `_bot_wujian_result_${ARM}.json`), JSON.stringify({ arm: ARM, runs, errs }, null, 2));
